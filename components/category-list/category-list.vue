<template>
	<view>
		<view class="nav">
			<!-- 左侧 -->
			<view class="nav-left">
				<scroll-view scroll-y scroll-with-animation :scrollTop="scrollLeftTop" :style="{ height: height }">
					<!-- TODO: 可自行修改样式，绑定事件等 -->
					<view
						class="nav-left-item"
			...
			<view class="nav-right">
				<scroll-view @scroll="scrollRight" :scroll-top="scrollTop" scroll-y :height="height" :style="{ height: height }" scroll-with-animation>
					...
					>
						<view v-for="(item, index) in categoryList" :key="index">
							<index-anchor :customStyle="{ right: 'auto', left: 'auto',backgroundColor:'#fff',height: '100rpx' }" :index="item.name" :useSlot="true">
								<view style="display: flex;align-items: center; justify-content: center; width: 70%">
									<view style="height: 1rpx; border-bottom: #f0f0f0 solid 1rpx; width: 20%;"></view>
									<view style="text-align: center; margin: 0 20rpx;color: #333; font-size: 28rpx; font-weight: 700;">{{ item.name }}</view>
